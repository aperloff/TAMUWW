...
void getHistosFromPlotterOutput(TH2D *& hd, TH2D *& hw, TH2D *& hq, int rebinPt, int rebinMet, bool full){

  bool removesw2 = false;// true;

  // list of background processes. QCD is handled separately
  ...
  for (unsigned int b = 0 ; b < backs.size();b++){
    ...
    if (removesw2) aux->GetSumw2()->Set(0);
...
void QCD_PtDependent_MetFit_Minuit(bool full = true){
  ...
  // Data, WJets, QCD
  TH2D * hd, *hw, *hq;
  getHistosFromPlotterOutput(hd,hw,hq,rebinPt,rebinMet,full);
  ...
  // The integral in wj
  TH1D * wj_int = hd->ProjectionX("mc_int");
  wj_int->SetTitle("mc_int");
  wj_int->Reset();
  ...
  TH1D * wj_nev = hd->ProjectionX("mc_nev");
  wj_nev->SetTitle("mc_nev");
  wj_nev->Reset();
  ...
  TH1D * wj_fr = hd->ProjectionX("mc_fr");
  wj_fr->SetTitle("mc_fr");
  wj_fr->Reset();
  ...
  TH1D * wj_yield = hd->ProjectionX("mc_yield");
  wj_yield->SetTitle("mc_yield;p_{T} [GeV];number of events / 19148 fb^{-1}");
  wj_yield->Reset();
  ...
  TH1D * wj_xs = hd->ProjectionX("mc_xs");
  wj_xs->SetTitle("mc_xs");
  wj_xs->Reset();
  ...
  // The wjets production xs according to the fit 
  TH1D * wj_sf = hd->ProjectionX("mc_sf");
  wj_sf->SetTitle(";p_{T} [GeV];N^{MC}_{measured} /N^{MC}_{expected} ");
  wj_sf->Reset();
  ...
  // The wjets production xs according to the fit 
  TH1D * wj_close = hd->ProjectionX("mc_close");
  wj_close->SetTitle("mc_close;p_{T} [GeV]");
  wj_close->Reset();
  ...
  for (int ipt = min_ipt; ipt <= max_ipt ; ipt++){
    ...
    minuit.mnparm( 1, "mc_sf", 0.75 , 0.01 , 0 , 1 , ierflg); // start at 75%
	...
	+ 1.0/met_w->GetEntries();
      ...
      cout<<"\t ipt="<<ipt<<" mc closure ="<< wj_close->GetBinContent(ipt)
  ...
  // ============ CREATE THE OUTPUT HISTOS ==============
  TFile *fout = new TFile("PtWeight_electron.root","RECREATE");
  
  TH1 * qcdweight = (TH1*) qcd_sf->Clone("QCDWeight_electron");
  TH1 * mcweight = (TH1*) wj_sf->Clone("MCWeight_electron");
  qcdweight->SetDirectory(0);
  mcweight->SetDirectory(0);
  qcdweight->Write();
  mcweight->Write();
  fout->Close();
